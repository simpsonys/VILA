...
// 생성될 로그 파일 경로
const destFile = path.join(__dirname, '..', 'test_cmd_mockapp_large.log');
...
for (let i = 1; i <= 100; i++) {
    ...
    if (missingType !== 'NO_ACTION') {
        stream.write(`${ts} I/VOICE_CLIENT ( 1000): Action: Mock.AudioPlayer.Play\n`);
        stream.write(`${ts} I/VOICE_CLIENT ( 1000): URL: https://mock.audio/play/track_${i}\n`);
